...
def main():
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id == 1:
                    ...
                    point_history.append(landmark_list[2])
                    ...
                    point_history.append(landmark_list[6])

                    point_history.append(landmark_list[10])

                    point_history.append(landmark_list[14])
                    point_history.append(landmark_list[18])
                ...
                if finger_gesture_id == 4:

                    keyboard.press(Key.right)
                    keyboard.release(Key.right)


                if finger_gesture_id ==5:

                    keyboard.press(Key.left)
                    keyboard.release(Key.left)

                if finger_gesture_id ==6:
        ...
        #image_lst.append(debug_image) gif saver

    cap.release()
    cv.destroyAllWindows()
    #imageio.mimsave('.//gif_exp//video.gif', image_lst, fps=60) gif saver
    ...
    for index, landmark in enumerate(landmark_point):
        ...
        if index == 3:  # Thumb 1st joint
            cv.circle(image, (landmark[0], landmark[1]), 5, (255, 255, 255),
                      -1)
            cv.circle(image, (landmark[0], landmark[1]), 5, (0, 0, 0), 1)
        if index == 4:  # Thumb: fingertip joint
